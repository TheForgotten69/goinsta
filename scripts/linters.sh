...
function fmt() {
  ...
  if [[ ${fmts} ]]; then
    ...
    gofmt -s -d $(find . -type f -name '*.go' | grep -v 'vendor' | grep -v '.git')
...
function go-group() {
  ...
  if [[ -f "$(go env GOPATH)/bin/gogroup" ]] || [[ -f "/usr/local/bin/gogroup" ]]; then
    declare -a lints=$(gogroup -order std,other,prefix=github.com/TheForgotten69/ $(find . -type f -name "*.go" | grep -v "vendor/"))
